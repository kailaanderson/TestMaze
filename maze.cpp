...
bool matchPair(const RoomPair& one, const RoomPair& two) {
	RoomPair first = one; 
	RoomPair second = two; 
	/*(matchRoom(first.one, second.one) || matchRoom(first.one, second.two)) {
		return true;
	}
	else if (matchRoom(first.two, second.one) || matchRoom(first.two, second.two)) {
		return true;
	}
	else return false; */
...
void printMaze(const RoomPair maze[]) {
	...
	//f (maze[wall].one.y != '*' || maze[wall].two.y != '*') {
			printPair(maze[wall]); 
		//}
	}
}
